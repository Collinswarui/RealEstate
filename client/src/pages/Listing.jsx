...
export default function Listing() {
  ...
  return (
    <main>
        ...
        text-2xl'>Loading...</p>}
        {/* Show error when there is one  */}
        {error && <p className='text-center my-7 
        text-2xl'>Something went wrong</p>}

        {/* If there is no error and no loading show the Estate */}
        ...
        <div className='flex flex-col max-w-4xl mx-auto p-3 my-7 gap-4'>
            ...
            {currentUser && listing.userRefs !== currentUser._id
            && !contact && (
            <button onClick={() => setContact(true)} className='bg-slate-700 text-white
            rounded-lg uppercase hover:opacity-90 mt-3 p-3'>
                Contact Owner
            </button>
            )}
